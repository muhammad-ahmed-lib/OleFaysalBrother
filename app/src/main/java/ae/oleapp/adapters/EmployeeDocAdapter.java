...
public class EmployeeDocAdapter extends RecyclerView.Adapter<EmployeeDocAdapter.ViewHolder>{
    ...
    private final List<EmployeeDocModel> list;
    private ItemClickListener itemClickListener;
    private String selectedId = "";


    public EmployeeDocAdapter(Context context, List<EmployeeDocModel> list) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (!list.get(position).getPhotoUrl().isEmpty()){
                Glide.with(context).load(list.get(position).getPhotoUrl()).apply(RequestOptions.circleCropTransform()).into(holder.imageView);
        }
