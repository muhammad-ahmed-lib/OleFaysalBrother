...
public class UniversalDayAdapter extends RecyclerView.Adapter<UniversalDayAdapter.ViewHolder> {

    private final Context context;
    private final List<Days> list;
    private ItemClickListener itemClickListener;
    private String selectedId = "";
    private int selectedIndex = -1;
    private final List<Days> selectedDaysIds =  new ArrayList<>();

    public UniversalDayAdapter(Context context, List<Days> list) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Days club = list.get(position);
        holder.tvName.setText(club.getName());

        if (isExist(list.get(position).getId()) != -1) {
    ...
    public List<Days> getSelectedDaysIds() {
        return selectedDaysIds;
    }


    public void selectClubs(Days day) {
        int index = isExist(day.getId());
        if (index == -1) {
            selectedDaysIds.add(day);
        }
        else {
            selectedDaysIds.remove(index);
        }
        notifyDataSetChanged();
    }

    public int isExist(int dayId) {
        int index = -1;
        for (int i = 0; i < selectedDaysIds.size(); i++) {
            if (selectedDaysIds.get(i).getId() == dayId) {
                index = i;
                break;
            }
        }
