...
public class ProductColorAdapter extends RecyclerView.Adapter<ProductColorAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.product_color, parent, false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ProductColor productColor = list.get(position);
        holder.bgVu.setCardBackgroundColor(ColorUtils.setAlphaComponent(Color.parseColor(productColor.getCode()), 52));
        holder.colorVu.setCardBackgroundColor(Color.parseColor(productColor.getCode()));
