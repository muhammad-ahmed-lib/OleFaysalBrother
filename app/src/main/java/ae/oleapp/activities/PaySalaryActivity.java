...
import ae.oleapp.databinding.ActivityPaySalaryBinding;
...
public class PaySalaryActivity extends BaseActivity implements View.OnClickListener {

    private ActivityPaySalaryBinding binding;
    ...
    private String clubId="", clubBankListItemId = "",  photoFilePath = "", employeeId="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        employeeSalaryAdapter = new EmployeeSalaryAdapter(getContext(), employeeSalaries, true);
        ...
        binding.etSalaryMonth.setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (v == binding.btnSubmit){
            if (binding.etPaymentMethod.getText().toString().isEmpty()){
                Functions.showToast(getContext(),"Please Select Payment Method", FancyToast.ERROR, FancyToast.LENGTH_SHORT);
                return;
            }
             if (binding.etSalary.getText().toString().isEmpty()){
                Functions.showToast(getContext(),getString(R.string.enter_salary), FancyToast.ERROR, FancyToast.LENGTH_SHORT);
                return;
            }
             if (binding.etSalaryMonth.getText().toString().isEmpty()) {
                Functions.showToast(getContext(),getString(R.string.select_month), FancyToast.ERROR, FancyToast.LENGTH_SHORT);
                return;
            }

            payEmployeeSalary(true, binding.etSalary.getText().toString(), binding.etSalaryMonth.getText().toString(), binding.etNote.getText().toString(),file);
        ...
        else if (v == binding.etSalaryMonth) {
            dateClicked();
        }
    ...
    EmployeeSalaryAdapter.ItemClickListener EmployeeSalaryitemClickListener = new EmployeeSalaryAdapter.ItemClickListener() {
        ...
        public void itemClicked(View view, int pos) {
            ...
            selectedIndex = pos;
            employeeId = employeeSalaries.get(pos).getId();
            employeeSalaryAdapter.setSelectedId(employeeId);
            employeeSalaryAdapter.setSelectedIndex(pos);
            //employeeClicked(employeeId);
            binding.etSalary.setText( employeeSalaries.get(pos).getSalary());
    ...
    private void dateClicked() {
        ...
        DatePickerDialog pickerDialog = new DatePickerDialog(getContext(), R.style.datepicker, new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                ...
                binding.etSalaryMonth.setText(formatter.format(calendar.getTime()));
            }
        },
    ...
    private void payEmployeeSalary(boolean isLoader, String salary, String month, String note, File file) {
        String userId = Functions.getPrefValue(getContext(), Constants.kUserID);
        if (userId!=null){
            ...
            Call<ResponseBody> call = AppManager.getInstance().apiInterface.payEmployeeSalary(part,
                    ...
                    RequestBody.create(MediaType.parse("multipart/form-data"),employeeId),
                ...
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    ...
                    if (response.body() != null) {
                        try {
                            JSONObject object = new JSONObject(response.body().string());
                            if (object.getInt(Constants.kStatus) == Constants.kSuccessCode) {
                                Functions.showToast(getContext(), getString(R.string.salary_paid), FancyToast.SUCCESS);
                                finish();
                            }
                            else {
                                Functions.showToast(getContext(), object.getString(Constants.kMsg), FancyToast.ERROR);
                    ...
                    try {
                        ...
                        if (object.getInt(Constants.kStatus) == Constants.kSuccessCode) {
                            ...
                            for (int i = 0; i < arr.length(); i++) {
                                employeeSalaries.add(gson.fromJson(arr.get(i).toString(), EmployeeSalary.class));
                            }
